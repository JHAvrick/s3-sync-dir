...
async function syncWorkspace(s3, bucket, prefix = '', root){
    ...
    //Get all objects after prefix
    //Remove prefix and compare to contents of directory
    //Add prefix again and update
	...
	for (let i = 0; i < unmatched.length; i++){
		...
		try {
			...
			//if (onBeforeUpload(bucket, root, unmatched[i])){
                let key = makeKey(prefix, root, unmatched[i]);
				await SYNC.initUploadFile(s3, bucket, key, unmatched[i]);
...
async function handleMatchedDeleted(params){
	...
	for (let i = 0; i < matchedDeleted.length; i++){
		...
		try {
			...
			let key = makeKey(prefix, root, matchedDeleted[i].file);
			...
			if (localIsCurrent || tags.includesThisDevice() || !tags.includesMD5(fileMD5)) {
                ...
                await SYNC.initUploadFile(s3, bucket, key, filePath);
...
async function handleMatchedUnsynced(params){
	...
	for (let i = 0; i < unsynced.length; i++){
		try {
			...
			let key = makeKey(prefix, root, filePath);
